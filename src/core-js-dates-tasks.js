...
function getDayName(date) {
  ...
  const getWeekDay = daysOfWeek[currentDay];
  return getWeekDay;
}
...
function getNextFriday(date) {
  ...
  const currentNumberOfDay = currentDay.getUTCDay();

  let additionalDays = 5 - currentNumberOfDay;
  ...
  const nextFriday = new Date(
    currentDay.setDate(currentDay.getDate() + additionalDays)
  );
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = Math.floor(miliseconds / (1000 * 60 * 60 * 24)) + 1;
...
function formatDate(date) {
  ...
  const halfADay = hours > 11 ? 'PM' : 'AM';

  if (hours > 12) {
    hours -= 12;
  }

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${halfADay}`;
}
